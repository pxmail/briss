...
/**
 * 按历史日期的某一天来导出运动员所有的运动项目数据
 */
class day_export extends Action {
	// 表头背景颜色
	...
	public function exec(array $params = null) {
		...
		$evaluation->getByTrainee ( $params ['trainee_id'], $params ['training_date'] );
		...
		$data_eval [] = array (
				...
				$evaluation->create_time,
				$evaluation->desire,
				$evaluation->sleep,
				$evaluation->appetite,
				$pain_parts,
				$pain_grades,
				$evaluation->rpe_before,
				$evaluation->rpe_after,
				$evaluation->hrv_before,
				$evaluation->hrv_after,
				$evaluation->morning_pulse,
				'',
		...
		$this->temp_eval ( $objPHPExcel, $title, $data_eval, $trainee );
		
		// 所有运动项目数据
		$training = new training ();
		$trainings = $training->listByTrainee ( $params ['trainee_id'], $params ['training_date'] );
		
		//处理“训练数据评估报表”sheet
		$sheetname = '训练前后状态评估表';
		$objPHPExcel->setActiveSheetIndex (0); // 选定当前激活sheet
		$objPHPExcel->getActiveSheet ()->setTitle ($sheetname); // 设置sheet标题
		$this->temp_eval($objPHPExcel, $sheetname, $data_eval, $trainee);
		...
		$active = 1;
 		foreach ($data as $k => $v ) {
 			...
 			if (!empty($v['data'])) {
 				...
 				$objPHPExcel->setActiveSheetIndex ( $active ); // 选定当前激活sheet
 				$objPHPExcel->getActiveSheet()->setTitle ( $v['sheetname'] ); // 设置sheet标题
 				$fn = 'temp_' . $k;
 				$this->{$fn} ( $objPHPExcel, $v['sheetname'], $v['data'], $trainee ); // 填充数据到sheet
 				$active++;
 			}
		}
		
		// 生成的excel文件名“训练数据（运动员姓名-训练日期）”
		$filename = "体能训练数据（" . $trainee->name . "-" . $params ['training_date'] . ")";
